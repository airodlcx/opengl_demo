...
int main()
{
	...
	Shader skyboxShader("skybox.vl","skybox.fl");
...
//			model = glm::rotate(model,(float)glfwGetTime(), glm::vec3(0.5f, 1.0f, 0.0f));
			...
			//skybox draw
			skyboxShader.use();
			skyboxShader.setMat4("projection", projection); // note: currently we set the projection matrix each frame, but since the projection matrix rarely changes it's often best practice to set it outside the main loop only once.
